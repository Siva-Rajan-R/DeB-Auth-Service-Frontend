...
export const LandingHero = () => {
  ...
  const floatingBubbles = [
    ...
    { label:"Custom Fields", icon: <UserPlus className="text-blue-600" size={18} />, left:"88%", speed: 13, delay: 0.5 },
    { label:"Email OTP", icon: <Mail className="text-indigo-600" size={18} />, left:"12%", speed: 20, delay: 7 },
  ...
  return (
    <section id="hero" className="relative min-h-[88vh] flex items-center justify-center pt-12 pb-24 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        {floatingBubbles.map((bubble, idx) => (
          <motion.div
            key={idx}
            className="neu-flat absolute hidden md:flex items-center gap-2.5 px-4 py-2.5 text-xs font-bold text-slate-800"
      ...
      <div className="max-w-5xl mx-auto px-6 relative z-10 text-center space-y-8">
        ...
        <div className="flex justify-center mb-2">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full -cyan-200 text-cyan-800 text-xs font-semibold neu-flat"
          >
        ...
        >
          <button
            ...
            disabled={loading}
            className="neu-button w-full sm:w-auto px-8 py-4 rounded-2xl font-extrabold text-base text-cyan-600 transition-all flex items-center justify-center gap-3 disabled:opacity-60"
          >
            {loading ? 'Launching...' :"Start Building — It's Free"}
            <ArrowRight size={18} />
          </button>
          <button
            onClick={() => navigate('/auth-docs')}
            className="neu-button w-full sm:w-auto px-8 py-4 rounded-2xl font-bold text-base text-[var(--text-muted)] hover:text-cyan-600 transition-all flex items-center justify-center gap-2"
          >

...
export const Hero = ({ scrollYProgress }) => {
  ...
  return (
    <div className="relative h-100 flex flex-col justify-center items-center rounded-2xl mx-3 mt-30 max-sm:mt-10 max-lg:mt-15 border-l border-r border-cyan-100 overflow-hidden">
        ...
        <motion.div style={{ y: yText, opacity: opacityText }} className='w-240 text-center z-[90] max-sm:w-80 relative'>
            <h1 className='text-[70px] max-sm:text-4xl max-lg:text-5xl max-sm:mb-2 max-lg:mb-2 text-slate-800 font-extrabold tracking-tight drop-shadow-sm'>
                Lock, Load, <span className="text-[#00d2e5]">Login</span>
            </h1>
        ...
        <motion.div style={{ y: yText, opacity: opacityText }} className='flex w-150 justify-center gap-6 mt-10 z-[90]'>
            <IceBlueButton btnName={"Grab API Key"} btnClassName={'text-[15px] font-semibold'} btnDivClassName={"max-sm:hidden shadow-lg shadow-cyan-200"}></IceBlueButton>
            <IceBlueButton btnName={"Get Started"} btnClassName={'text-[15px] font-semibold'} onclickFunc={()=>{window.open('/auth-docs','_blank')}} btnDivClassName={"shadow-lg shadow-cyan-200"}></IceBlueButton>
        </motion.div>
